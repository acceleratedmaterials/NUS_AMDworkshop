...
class NeuralGraphHidden(layers.Layer):
    ...
    def __init__(self, inner_layer_arg, **kwargs):
        ...
        if isinstance(inner_layer_arg, (int, int)):
            '''
            ...
            '''
            raise TypeError('Use method 3 to initialise')
        ...
        # Case 3: Check if a function is provided that returns a initialised keras layer
            ...
            # _, self.conv_width = example_instance.get_output_shape_for((None, None))  # OLD CODE
            self.conv_width = example_instance.units  # same as output graph function. See comments there
    ...
    def call(self, inputs, mask=None):
        ...
        # atom_degrees = K.sum(K.not_equal(edges, -1), axis=-1, keepdims=True)  # OLD
        ...
        # new_features = layers.merge(new_features_by_degree, mode='sum')  # OLD
    ...
    def get_output_shape_for(self, inputs_shape):
        ...
        (max_atoms, max_degree, num_atom_features, num_bond_features,
         num_samples) = mol_shapes_to_dims(mol_shapes=inputs_shape)
        raise TypeError('Use change code at this line to compute_output_shape instead')
...
class NeuralGraphOutput(layers.Layer):
    ...
    def __init__(self, inner_layer_arg, **kwargs):
        ...
        # Case 3: Check if a function is provided that returns a initialised keras layer
            ...
            assert isinstance(example_instance,
                              layers.Layer), 'When initialising with a function, the function has to return a keras layer'
            assert example_instance.built == False, 'When initialising with a keras layer, it cannot be built.'
            self.fp_length = example_instance.units  # Since we just want the output vector size, just take output units
    ...
    def call(self, inputs, mask=None):
        ...
        # EDIT: Old version. sum cannot work with bool, so change bool to float
        # atom_degrees = K.sum(K.not_equal(edges, -1), axis=-1, keepdims=True)  # not_equal returns bool tensor
        # print(K.cast(K.not_equal(edges, -1), K.floatx()))  # floatx() returns the float type Keras uses as a string

        # atom_degrees is a max_atom x 1 vector. Each element represents the number of other atoms that atom is bonded
        atom_degrees = K.sum(K.cast(K.not_equal(edges, -1), K.floatx()), axis=-1, keepdims=True)
        # general_atom_mask is max_atom x 1. Each element is either 1 or 0, meaning has connection or no connection
        general_atom_mask = K.cast(K.not_equal(atom_degrees, 0), K.floatx())

        # Sum the edge features for each atom. Sum up all bond features connected to 1 particular atom
        summed_bond_features = K.sum(bonds, axis=-2)
        ...
        # print(final_fp_out.shape)  # returns (?,100) which is correct for test run of fp_size=100
        return final_fp_out
