...
from keras.regularizers import L1L2 as l1l2  # l1l2 changed to l1_l2 in keras2
...
def build_graph_conv_net_fp_only(data_input,
    ...
    # Added list in front of zip_mixed for python3 compatibility
    ...
    if len(fp_layer_sizes) != len(conv_layer_sizes) + 1:
        assert len(
            fp_layer_sizes) == 1, 'Incorrect amount of fingerprint layers specified. Either specify 1 or len(conv_layer_sizes)+1 ({}) fp_layer_sizes ({})'.format(
    ...
    assert len(conv_layers) == len(
        conv_layer_sizes), 'If `conv_`-layer-arguments are specified as a list, they should have the same length as `conv_layer_sizes` (length {0}), found an argument of lenght {1}'.format(
        len(conv_layer_sizes), len(conv_layers))
    assert len(fp_layers) == len(
        fp_layer_sizes), 'If `fp`-layer-arguments are specified as a list, they should have the same length as `fp_layer_sizes` (len {0}), found an argument of lenght {1}'.format(
    ...
    if fp_size:
        ...
        def inner_layer_fn():
            return Dense(fp_size, activation=fp_activation, bias=fp_bias,
                         W_regularizer=l1l2(fp_l1, fp_l2),
                         b_regularizer=l1l2(fp_l1, fp_l2), **fp_kwargs)
        fp_out = NeuralGraphOutput(inner_layer_fn)([fp_atoms_in, bonds, edges])
        fingerprint_outputs.append(fp_out)

    # Add Graph convolutional layers
    convolved_atoms = [atoms]
    # First layer fp hparam already popped, so iterating through the remaining layers
    for conv_layer, fp_layer in zip(conv_layers, fp_layers):
        ...
        # print(atoms_in.shape)  # (no. of examples, max atom, atom features or conv_width of previous hidden conv)
        if conv_batchnorm:
        ...
        # If using leakyrelu activation function, you must edit the main keras file to add this function
        # keras.activations    add the following function
        # def leakyrelu(x, alpha=0.01, max_value=None):
        #     return K.relu(x, alpha=alpha, max_value=max_value)
        # adjust the alpha value to the value you want.
        # Using method 2 to initialise
        atoms_out = NeuralGraphHidden(inner_layer_fn)([atoms_in, bonds, edges])
        # print('out',atoms_out.shape)  # (no. of examples, max atom, conv_width)
    ...
    if len(fingerprint_outputs) > 1:
        # final_fp = merge(fingerprint_outputs, mode=fp_merge_mode)  # OLD
        final_fp=layers.Add()(fingerprint_outputs)
    else:
...
def build_graph_conv_net(data_input,
    ...
    fp_layers = list(zip_mixed(fp_layer_sizes, fp_activation, fp_bias,
                               fp_l1, fp_l2, fp_dropout, fp_batchnorm,
                               fp_kwargs, repeat_classes=[dict, str]))
    net_layers = list(zip_mixed(net_layer_sizes, net_activation, net_bias,
    ...
    assert len(net_layers) == len(
        net_layer_sizes), 'If `net_`-layer-arguments are specified as a list, they should have the same length as `net_layer_sizes` (length {0}), found an argument of lenght {1}'.format(
